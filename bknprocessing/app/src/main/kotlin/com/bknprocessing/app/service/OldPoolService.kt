...
import com.bknprocessing.app.utils.endNetworkVerify
import com.bknprocessing.app.utils.logger
import com.bknprocessing.app.utils.startNetworkVerify
import com.bknprocessing.app.utils.startNode
import com.bknprocessing.app.utils.waitAllChannelEmpty
...
open class OldPoolService(
    ...
    protected val nodesCountBlocksGenerated = mutableListOf<Long>()
    ...
    init {
        ...
        for (idx in 0 until nodesCount - unhealthyNodesCount) {
            nodes.add(OldNode(index = nodeIndex, isHealthy = true, createdAt = createdAt))
            nodesCountBlocksGenerated.add(0)
            nodeIndex += 1
        }
        for (idx in 0 until unhealthyNodesCount) {
            nodes.add(OldNode(index = nodeIndex, isHealthy = false, createdAt = createdAt))
            nodesCountBlocksGenerated.add(0)
            nodeIndex += 1
        }
    }

    private fun countBlocksCreatedByNodeInChain(nodeId: UUID): Long =
        nodesCountBlocksGenerated[nodes.first { it.id == nodeId }.index]
    ...
    @OptIn(ObsoleteCoroutinesApi::class)
    private suspend fun doMine(node: INodeMiner) {
        if (node.isMiner()) {
            while (!isFinished) {
                delay(DELAY_MILSECS)
    ...
    @OptIn(ObsoleteCoroutinesApi::class)
    private suspend fun doVerify(node: INodeVerifier) {
        ...
        while (!isFinished) {
            ...
            if (verificationDto.nodeIndex == (node as INode).index) continue
